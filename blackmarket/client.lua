...
if Config.UseTimer then
    CreateThread(function()
        ...
        while true do
            ...
            minutes = minutes - 1
            if minutes == 0 then
                TriggerEvent("aendir-blackmarket:client:removeall")
                mainthread()
...
function mainthread()
    for k, v in pairs(Config.Locations) do
        ...
        if Config.RandomLocation then
            ...
            exports['qb-target']:AddCircleZone("['"..k.."("..m..")']", vector3(v["coords"][m].x, v["coords"][m].y, v["coords"][m].z), 2.0, { name="['"..k.."("..m..")']", debugPoly=Config.Debug, useZ=true, },{ options = { { event = "aendir-blackmarket:ShopMenu", icon = "fas fa-certificate", label = Lang:t("target.browse"),item = (v.openwith or nil),gang = (v.gang or nil), shoptable = v, products = productstable, name = v["label"], k = k, l = m, }, }, distance = 2.0 })
        else
            for l, b in pairs(v["coords"]) do -- Create ped for each location given in Config
                ...
                exports['qb-target']:AddCircleZone("Shop - ['"..k.."("..l..")']", vector3(b.x, b.y, b.z), 2.0, { name="Shop - ['"..k.."("..l..")']", debugPoly=Config.Debug, useZ=true, },{ options = { { event = "aendir-blackmarket:ShopMenu", icon = "fas fa-certificate", label = Lang:t("target.browse"), item = (v.openwith or nil),gang = (v.gang or nil), shoptable = v, products = productstable, name = v["label"], k = k, l = l, }, }, distance = 2.0 })
            end
        end
    end
end

RegisterNetEvent('aendir-blackmarket:ShopMenu', function(data, custom)
    ...
    ShopMenu[#ShopMenu + 1] = { header = "", txt = Lang:t("menu.close"), params = { event = "aendir-blackmarket:CloseMenu" } }
    ...
    for i = 1, #products do
            ...
            for i2 = 1, #products[i].requiredJob do
                if QBCore.Functions.GetPlayerData().job.name == products[i].requiredJob[i2] then
                    ShopMenu[#ShopMenu + 1] = { icon = products[i].name, header = setheader, txt = text, isMenuHeader = lock,
                        params = { event = "aendir-blackmarket:Charge", args = { item = products[i].name, cost = totalprice, info = products[i].info, shoptable = data.shoptable, k = data.k, l = data.l, amount = amount, custom = custom, rem = rmi} } }
                end
            end
        elseif products[i].requiresLicense then
            ...
            ShopMenu[#ShopMenu + 1] = { icon = products[i].name, header = setheader, txt = text, isMenuHeader = lock,
                    params = { event = "aendir-blackmarket:Charge", args = {
...
RegisterNetEvent('aendir-blackmarket:CloseMenu', function() exports['qb-menu']:closeMenu() end)

RegisterNetEvent('aendir-blackmarket:Charge', function(data)
    ...
    if dialog then
        ...
        if tonumber(dialog.amount) <= 0 then TriggerEvent("QBCore:Notify", Lang:t("error.incorrect_amount"), "error") TriggerEvent("aendir-blackmarket:Charge", data) return end
        if data.cost == "Free" then data.cost = 0 end
        TriggerServerEvent('aendir-blackmarket:GetItem', dialog.amount, dialog.billtype, data.item, data.shoptable, data.cost, data.rem)
...
RegisterNetEvent("aendir-blackmarket:client:removeall",function()
    for k, v in pairs(Config.Locations) do
        if Config.RandomLocation then
